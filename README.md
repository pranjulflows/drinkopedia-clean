...
The app is designed around that: a curated seed list of 44 verified spirit names
(`assets/data/spirits_seed.json`) is hydrated name by name and cached in SQLite. Browse reads
from the cache, never from a capped endpoint — which also means it works offline and stays
off a rate-limited key.
